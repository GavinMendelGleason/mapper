"""
Chunking utilities for cable text processing.

This module provides functionality to split cables into chunks
for efficient processing and embedding generation.
"""
...
class CableChunk(BaseModel):
    """A chunk of text from a cable with its metadata."""
...
def chunk_cable(
    ...
    """
    Split a cable into chunks for embedding generation.

    Args:
        cable_id: The ID of the cable
        content: The text content of the cable
    ...
    phrasal_chunks = chunk_phrases(phrases, chunk_size)
    
    # Create CableChunk objects
    cable_chunks = []
    for i, chunk in enumerate(phrasal_chunks):
        cable_chunk = CableChunk(
            cable_id=cable_id,
            chunk_id=i,
            text=chunk.text,
        ...
        if metadata:
            if "date" in metadata:
                cable_chunk.date = metadata["date"]
            if "source" in metadata:
                cable_chunk.source = metadata["source"]
            if "classification" in metadata:
                cable_chunk.classification = metadata["classification"]
        
        cable_chunks.append(cable_chunk)
